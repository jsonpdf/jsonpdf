...
const defaultStyle: Style = { fontFamily: 'Inter' };
...
describe('createMeasureContext', () => {
  it('creates context with correct availableWidth and height', () => {
    ...
    const ctx = createMeasureContext(el, fonts, {}, defaultStyle, pdfDoc, imageCache);
  ...
  it('applies named style with padding', () => {
    ...
    const ctx = createMeasureContext(el, fonts, styles, defaultStyle, pdfDoc, imageCache);
  ...
  it('uses custom defaultStyle in resolved element style', () => {
    const el = makeElement();
    const custom: Style = { fontFamily: 'Custom', fontSize: 16, color: '#112233' };
    const ctx = createMeasureContext(el, fonts, {}, custom, pdfDoc, imageCache);
    expect(ctx.elementStyle.fontFamily).toBe('Custom');
    expect(ctx.elementStyle.fontSize).toBe(16);
    expect(ctx.elementStyle.color).toBe('#112233');
  });

  it('named style overrides defaultStyle values', () => {
    const el = makeElement({ style: 'heading' });
    const custom: Style = { fontFamily: 'Custom', fontSize: 16 };
    const ctx = createMeasureContext(el, fonts, styles, custom, pdfDoc, imageCache);
    // heading sets fontSize: 24, which overrides defaultStyle fontSize: 16
    expect(ctx.elementStyle.fontSize).toBe(24);
    // fontFamily comes from defaultStyle since heading doesn't set it
    expect(ctx.elementStyle.fontFamily).toBe('Custom');
  });
});
